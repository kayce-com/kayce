...
contract StateMachine {
     ...
     */
    function transitionExists(bytes32 originState, bytes32 targetState) public view virtual returns (bool)
    {
     ...
     */
    function _createTransition(bytes32 originState, bytes32 targetState) internal {
        require(currentState == SETUP_STATE, "State machine not in SETUP.");
        require(!transitionExists(originState, targetState),"Transition already exists.");
    ...
    function _transition(bytes32 targetState) internal {
        require(transitionExists(currentState, targetState),"Transition doesn't exist.");
